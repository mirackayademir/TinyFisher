...
# Efektler dünya katmanında ama deniz arka planının üstünde, Leviathan'ın altında çizilir.
const WAKE_TAIL_OFFSET_X: float = 105.0
const WAKE_TAIL_OFFSET_Y: float = 8.0
const WAKE_MAX_POINTS: int = 34
const WAKE_SAMPLE_SECONDS: float = 0.07
const BUBBLE_BASE_SECONDS: float = 0.10
const BUBBLE_MIN_RADIUS: float = 3.0
const BUBBLE_MAX_RADIUS: float = 8.0
...
func _configure_visual(fish: Area2D) -> void:
	...
	# Su izi/kabarcıklar görünür kalsın ama balığın üstüne binmesin.
	sprite.z_index = 4
...
func _setup_wake_effects() -> void:
	...
	_wake_outer.width = 22.0
	_wake_outer.default_color = Color(0.18, 0.68, 0.88, 0.34)
	...
	_wake_inner.width = 7.0
	_wake_inner.default_color = Color(0.72, 0.94, 1.0, 0.52)
	_wake_inner.antialiased = true
	_wake_inner.z_index = 3
	...
	# İlk anda da ekranda efekt görülsün; balığın biraz hareket etmesini beklemeyelim.
	for i in range(4):
...
func _update_wake_effects(delta: float) -> void:
	...
	if _bubble_timer <= 0.0:
		...
		var speed_factor: float = clampf(velocity_value / 30.0, 0.80, 1.80)
...
func _apply_wake_points() -> void:
	...
	var visibility_ratio: float = speed_ratio / 1.8
	_wake_outer.width = lerpf(15.0, 28.0, visibility_ratio)
	_wake_inner.width = lerpf(5.0, 10.0, visibility_ratio)
	_wake_outer.default_color = Color(0.18, 0.68, 0.88, lerpf(0.23, 0.42, visibility_ratio))
	_wake_inner.default_color = Color(0.72, 0.94, 1.0, lerpf(0.38, 0.62, visibility_ratio))
...
func _get_tail_global_position() -> Vector2:
	...
	var tail_offset := Vector2(
		...
		WAKE_TAIL_OFFSET_Y + sin(_effect_time * 2.2) * 5.0
	)
...
func _spawn_bubble_cluster(speed_factor: float) -> void:
	...
	if speed_factor > 1.10 and randf() < 0.65:
		bubble_count = 3
	if speed_factor > 1.45 and randf() < 0.40:
		bubble_count = 4
...
func _spawn_single_bubble(index: int) -> void:
	...
	bubble.width = randf_range(1.8, 3.2)
	bubble.default_color = Color(0.78, 0.95, 1.0, randf_range(0.58, 0.82))
	...
	var spawn_global: Vector2 = _get_tail_global_position() + Vector2(
		backwards_sign * randf_range(4.0, 22.0) + backwards_sign * float(index) * 5.0,
		randf_range(-20.0, 20.0)
	)
	bubble.position = _world.to_local(spawn_global)
	bubble.scale = Vector2.ONE * randf_range(0.85, 1.25)

	var drift := Vector2(
		backwards_sign * randf_range(22.0, 58.0),
		-randf_range(40.0, 78.0)
	)
	var life: float = randf_range(0.95, 1.55)
