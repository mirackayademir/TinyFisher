...
func spawn_one_fish() -> void:
	...
	if roll <= 45:
		...
		new_fish.swim_speed = 70.0

	elif roll <= 75:
		new_fish.fish_type = "Levrek"
		new_fish.fish_value = 25
		new_fish.swim_speed = 55.0

	elif roll <= 92:
		new_fish.fish_type = "Uskumru"
		new_fish.fish_value = 40
		new_fish.swim_speed = 85.0

	else:
		new_fish.fish_type = "Ton Balığı"
		new_fish.fish_value = 75
		new_fish.swim_speed = 45.0

	# Set the spawn position before _ready records the patrol centre.
	add_child(new_fish)
